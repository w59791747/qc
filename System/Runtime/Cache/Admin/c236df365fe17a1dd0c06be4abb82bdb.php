      ...
      <table class="table_data daemon_data" cellpadding="0" cellspacing="0">
        <tr>
         ...
         <?php if(is_array($list)): foreach($list as $key=>$var): ?><tr>
                ...
                <td class="w-200">
                    <?php if(($var["is_open"]) == "1"): echo ROLE('payment/uninstall',array("payment_id"=>$var["payment_id"]),'卸载','act','pub_btn grayBtn');?>
                     <?php echo ROLE('payment/install',array("payment_id"=>$var["payment_id"]),'编辑','','pub_btn');?>
            <?php else: ?>
            <?php echo ROLE('payment/install',array("payment_id"=>$var["payment_id"]),'安装','','pub_btn'); endif; ?>
            
            <?php if($var["is_mobile_only"] == 1): echo ROLE('payment/closemobile',array("payment_id"=>$var["payment_id"]),'关闭手机','act','pub_btn grayBtn');?>
            <?php else: ?>
            <?php echo ROLE('payment/closemobile',array("payment_id"=>$var["payment_id"]),'开启手机','act','pub_btn'); endif; ?>
            </td>
