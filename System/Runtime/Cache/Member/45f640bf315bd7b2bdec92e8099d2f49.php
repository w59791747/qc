...
<div class="uc_mid_r">
			...
			<div class="cz_cont">
             <?php if($code == lianlian): ?><form method="post"  action="<?php echo U('payment/order');?>">
               
             
             <?php else: ?>
            	<form method="post"  action="<?php echo U('payment/order');?>" target="chuang_frm"><?php endif; ?>
				<p><span>请确认以下支付信息：</span></p>
				<p><span>充值来源：</span><?php echo ($types[$from]); ?></p>
                <p><span>充值方式：</span><?php echo ($payment[$code][name]); ?></p>
				<p><span>充值金额：</span>¥<?php echo ($number); ?></p>
                <?php if($log['jifen'] > 0): ?><p><span>使用积分：</span><?php echo ($log["jifen"]); ?></p>
                <p><span>实际支付：</span>¥<?php echo ($log[actual_pay]/100); ?></p><?php endif; ?>
                <input type="hidden" name="data[order_id]" value="<?php echo ($order_id); ?>" />
                <input type="hidden" name="data[code]" value="<?php echo ($code); ?>" />
                
                
                <?php if($code == money): ?><p><span>支付密码：</span><input type="password" name="data[pwd]" ></p><?php endif; ?>
				<input type="submit" class="cz_but" value="立刻支付">
                </form>
