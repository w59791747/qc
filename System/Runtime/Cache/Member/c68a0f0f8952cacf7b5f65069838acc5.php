...
<div class="uc_mid_r yitou_list">
			<ul class="cz_tit">
				<li><a href="<?php echo U('member/set/passwd');?>">修改密码</a></li>
				<li class="cz_cur"><a href="<?php echo U('member/set/renzheng');?>">账号认证</a></li>
          ...
          <?php if($MEMBER["verify"] == 4 || $MEMBER["verify"] == 5 || $MEMBER["verify"] == 6|| $MEMBER["verify"] == 7): ?><div class="warning"> <i></i>已经通过实名认证：<span class="fontcl2"><?php echo ($MEMBER["detail"]["realname"]); ?></span>
             </div>
          <?php elseif($renzheng == 1): ?>
             <div class="warning"> <i></i>您好，您的实名认证已经提交，请耐心等待后台审核…
             </div>
          <?php else: ?>
           <form method="post"  action="<?php echo U('set/name');?>"  target="chuang_frm">
			<div class="xgzl">
				<p><span>真实姓名</span><input type="text" name="data[realname]"   class="mima_box" /></p>
				<p><span>身份证号码</span><input type="text" name="data[card]"   class="mima_box" /></p>
                ...
                <!--<p class="sfz">
                  ...
                  <div class="control_txt">
                    <img id="photo_img" width="100px;"  src="__ATTACHS__/<?php echo (($MEMBER["detail"]["card_zhen"])?($MEMBER["detail"]["card_zhen"]):'default.jpg'); ?>" />
                    <input type="hidden" name="data[card_zhen]" value="<?php echo ($MEMBER["detail"]["card_zhen"]); ?>" id="data_photo" />
                    <input class="xg_touxiang"  id="photo_file" name="photo_file" type="file" multiple="true" value="" />
                  </div>
                </p>
                 ...
                 <p class="sfz">
                  <span>身份证反面</span>
                  <div class="control_txt">
                    <img id="photo_img1" width="100px;"  src="__ATTACHS__/<?php echo (($MEMBER["detail"]["card_fan"])?($MEMBER["detail"]["card_fan"]):'default.jpg'); ?>" />
                    <input type="hidden" name="data[card_fan]" value="<?php echo ($MEMBER["detail"]["card_fan"]); ?>" id="data_photo1" />
                    <input class="xg_touxiang"  id="photo_file1" name="photo_file1" type="file" multiple="true" value="" />
                  </div>
                </p>-->
  ...
  <script type="text/javascript" src="__PUBLIC__/js/uploadify/jquery.uploadify.min.js?t=<?php echo ($nowtime); ?>"></script>
    ...
    <script type="text/javascript">
      	 $("#photo_file").uploadify({
            ...
            'uploader': '<?php echo U("up/upload/uploadify",array("model"=>"member_thumb"));?>;jsessionid=${pageContext.session.id}',
		...
		$("#photo_file1").uploadify({
            ...
            'onUploadSuccess': function (file, data, response) {
                ...
                $("#photo_img1").attr('src', '__ATTACHS__/' + data).show();
            }
        });
    </script>
