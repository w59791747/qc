...
<div class="uc_mid_r">
			<ul class="cz_tit">
				<li class="cz_cur"><a href="<?php echo U('member/priceup');?>">线上充值</a></li>
				<li><a href="<?php echo U('member/priceup2');?>">线下充值</a></li>
			</ul>
            <form method="post"  action="<?php echo U('member/pricepay');?>"  target="chuang_frm">
			<div class="cz_cont">
				<p><span>当前可用余额（元）：</span><?php echo ($MEMBER["price"]); ?></p>
                 <?php if($order): ?><input readonly="readonly"  type="hidden" name="data[order_id]"  value="<?php echo ($order[order_id]); ?>" />
            	 <p><span>充值金额（元）：<input readonly="readonly"  type="text" name="data[number]"  value="<?php echo ($order[amount]/100); ?>" class="text" /></p>
                 <?php else: ?>
				<p><span>请输入充值金额（元）：</span><input type="text /" name="data[number]" ></p><?php endif; ?>
				<p><span>请选择充值方式：</span></p>	
				<ul>	
                    
                    <?php if(is_array($payment)): foreach($payment as $key=>$var): if(($var["is_open"]) == "1"): if($var['code'] != 'money'): ?><li><label><input class="pay_int" value="<?php echo ($var["code"]); ?>" name="data[code]" type="radio" />
                           <img src="__PUBLIC__/images/<?php echo ($var["logo"]); ?>" alt="" /><label></li><?php endif; endif; endforeach; endif; ?>
				</ul>	
                <input type="submit" class="cz_but" value="立即充值">
                </form>
			</div>			
		</div>
		<div style="clear:both"></div>
