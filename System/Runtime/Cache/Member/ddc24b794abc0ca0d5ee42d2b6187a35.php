...
<div class="uc_mid_r yitou_list">
			<div class="select_yitou">
                ...
                <form method="post"  action="<?php echo U('project/order');?>">
                    <span>请选择时间：</span>
                    ...
                    <?php if(is_array($statuslist)): foreach($statuslist as $key=>$var): ?><option value="<?php echo ($key+2); ?>" <?php if($status == $key): ?>selected="selected"<?php endif; ?>><?php echo ($var); ?></option><?php endforeach; endif; ?>
                    </select>
             ...
             <?php if($list): ?><table width="100%" border="0" cellspacing="0" cellpadding="0">
				 <tbody>
					...
					<?php if(is_array($list)): foreach($list as $key=>$var): ?><tr>
                          ...
                          <td><?php echo ($var["order_no"]); ?></td>
                          <td>￥<?php echo ($var["amount"]); ?></td>
                          <td><?php echo (date("Y-m-d H:i:s",$var["dateline"])); ?></td>
                          <td ><?php echo ($statuslist[$var[pay_status]]); ?></td>
                         
                          <td>
                           <?php if($var["pay_status"] == 0): ?><a class="gozf" href="<?php echo U('member/orderpay',array('order_id'=>$var['order_id']));?>">支付</a>
                            <a target="chuang_frm" class="quxiao" mini="confirm" href="<?php echo U('member/orderdelete',array('order_id'=>$var['order_id']));?>">取消订单</a><?php endif; ?></td>
