			...
			<ul class="uc_nav">
				...
				<li class="uc_nav_li uc_nav_li_zh"><a href="javascript:void(0);">账号设置</a>
					<ul <?php if($ctl == set): ?>class="cur_uc_nav"<?php endif; ?>>
						<li><a href="<?php echo U('member/set/index');?>"
                        <?php if($ctl == set && $act == index): ?>class="cur_uc_nav_a"<?php endif; ?>
                        >修改资料</a></li>
...
<div class="uc_mid_r yitou_list">
			<div class="yitou_data">
				<p>已投数据概览</p>
				<ul>
					<li>累计投资（个）<span><?php echo ($count1); ?></span></li>
					<li>近7天投资（个）<span><?php echo ($count2); ?></span></li>
					<li>近30天投资（个）<span><?php echo ($count3); ?></span></li>
					<li style="border:none">累计投资金额（¥）<span><?php echo ($count4/100); ?></span></li>
				</ul>
			</div>
			<div class="select_yitou">
            	<form method="post"  action="<?php echo U('project/crowd_canyu');?>">
                    <span>请选择时间：</span>
                    <input  name = 'stime' value="<?php echo ($stime); ?>" type="text"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'});"/><span>到</span><input name = 'ltime' type="text" value="<?php echo ($ltime); ?>"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
                    <select name="status">
                    	<option value="0" <?php if($status == 0): ?>selected="selected"<?php endif; ?>>全部</option>
                        
                        <?php if(is_array($status_list)): foreach($status_list as $key=>$var): ?><option value="<?php echo ($key+3); ?>" <?php if($status == $key+3): ?>selected="selected"<?php endif; ?>><?php echo ($var); ?></option><?php endforeach; endif; ?>
            ...
            <?php if($list): ?><table width="100%" border="0" cellspacing="0" cellpadding="0">
				 <tbody>
					<tr>
						<th width="10%">ID</th>
						<th width="25%">项目标题</th>
					...
					<?php if(is_array($list)): foreach($list as $key=>$var): ?><tr>
                       <td><?php echo ($var[list_id]); ?></td>
                        <td><a target="_blank" href="<?php echo U('home/project/detail',array('project_id'=>$var[project_id]));?>"><?php echo ($project[$var[project_id]]["title"]); ?></a></td>
                        <td>￥<?php echo ($var[price]/100); ?></td>
                        
                       
                        <td><?php echo (date("Y-m-d H:i:s",$var["dateline"])); ?></td>
                        <td><?php if($var['status'] == 1): ?>已赞成
                        
                            <?php elseif($var['status'] == -1): ?>
                           已反对
                            <?php elseif($project[$var['project_id']]['status'] == 3): ?>
                            
                            <a  class="gozf" href="<?php echo U('home/project/detail',array('project_id'=>$var['project_id']));?>?toupiao">投票</a>
                            <?php else: ?>
                            <font class="black6">等待投票</font><?php endif; ?></td>
                            
                            <td>￥<?php echo ($var[shouyi]/100); ?></td>
                            <td><a href="<?php echo U('project/hetong',array('list_id'=>$var['list_id']));?>">下载</a></td>
			...
			<div class="yitou_none">
				...
				<div class="select_page us_center"><?php echo ($page); ?></div>
		</div>
		<div style="clear:both"></div>
	</div>
</div>
