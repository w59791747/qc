...
<div class="itemwrap">
  <div class="ad_area">
    <ul>
       <?php  $items = D("Advitem")->where("adv_id=99 and audit=1  and ((`stime`=0 && `ltime`=0) || (`stime`<1543565955&& `ltime`>1543565955))")->order("orderby asc")->limit("0,6")->select(); $index=0; foreach($items as $item){$index++; ?><li style="background:url(__ATTACHS__/<?php echo ($item["thumb"]); ?>) 50% 0 no-repeat;"  onclick="window.open('<?php echo ($item["link"]); ?>','<?php echo ($item["target"]); ?>')"></li> <?php } ?>
    </ul>
  </div>
	<div class="itemlist">
<!-- 		<h1>中国领先的二手车众筹平台，安全可靠</h1>
  ...
  <div class="ad_area">
    <ul>
       <?php  $items = D("Advitem")->where("adv_id=100 and audit=1  and ((`stime`=0 && `ltime`=0) || (`stime`<1543565955&& `ltime`>1543565955))")->order("orderby asc")->limit("0,6")->select(); $index=0; foreach($items as $item){$index++; ?><li style="background:url(__ATTACHS__/<?php echo ($item["thumb"]); ?>) 50% 0 no-repeat;"  onclick="window.open('<?php echo ($item["link"]); ?>','<?php echo ($item["target"]); ?>')"></li> <?php } ?>
    </ul>
  </div>
	<div class="itemlist">
		<!-- <h1>成功案例</h1>
