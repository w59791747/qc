...
<div class="banner">
  <div class="flexslider">
    <ul class="slides">
      <?php  $items = D("Advitem")->where("adv_id=101 and audit=1  and ((`stime`=0 && `ltime`=0) || (`stime`<1543470068&& `ltime`>1543470068))")->order("orderby asc")->limit("0,3")->select(); $index=0; foreach($items as $item){$index++; ?><li><a href="<?php echo ($item["link"]); ?>" target="<?php echo ($item["target"]); ?>"><img src="__ATTACHS__/<?php echo ($item["thumb"]); ?>" /></a></li> <?php } ?>
    </ul>
  </div>
</div>
