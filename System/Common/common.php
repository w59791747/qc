...
function setUid($uid) {
    ...
    cookie('zhuchuang_TOKEN', $uid, 86400 * 365); //存一年
    return true;
}

function clearUid() {
    cookie('zhuchuang_TOKEN', null); //存一年
    return true;
}

function getUid() {
    import("ORG/Crypt/Base64");
    $token = cookie('zhuchuang_TOKEN');
...
function zhuQrCode($token, $url, $size = 8) { //生成网址的二维码 返回图片地址
    $md5 = md5($token);
...
function zhu_name($str) {

    $str = preg_replace("@<(.*?)>@is", "", $str);
    return msubstr($str, 0, 3, 'utf-8',false).'***'.msubstr($str, -4, 4, 'utf-8',false);
}

function zc_int($str) {

    $str = preg_replace("@<(.*?)>@is", "", $str);
    return msubstr($str, 0, 5, 'utf-8',false);
}
...
//专门给含有HTML的字段
function zhu_msubstr($str, $start, $length, $suffix) {
 ...
 * @return string
 */
function ROLE($url = '', $vars = '', $title = '', $mini = "", $class = "", $width = '', $height = '') {
    static $admin;
    if (empty($admin)) {
        $admin = session('admin');
...
function cardstr($str) {
	$return = substr($str,0,5).'****'.substr($str,-4);
