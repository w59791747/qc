...
class WeixinJSSDK {
    ...
    public function getSignPackage() {
		...
		//var_dump($signPackage);echo "File:", __FILE__, ',Line:',__LINE__;exit;
        return $signPackage; 
    }
  ...
  private function getJsApiTicket() {
    ...
    if (!($data = json_decode(file_get_contents("jsapi_ticket.json"))) && $data['expire_time'] < time()) {
  ...
  private function getAccessToken() {
    ...
    if (!($data = json_decode(file_get_contents("access_token.json"))) && $data['expire_time'] < time()) {
  ...
  private function httpGet($url) {
    ...
    {
        // jsapi_ticket 应该全局存储与更新，以下代码以写入到文件中做示例    ;
		if (!($data = json_decode(file_get_contents("card_ticket.json"))) && $data['expire_time'] < time()) {
            $accessToken = $this->getAccessToken();
