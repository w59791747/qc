...
class WeixinModel {
    ...
    */
	...
	public function admin_wechat_client($weixin_a,$shop_id=0)
    {
        static $clients = array();
		if($weixin_admin = D('Shopdetails')->find($shop_id)){
			include_once APP_PATH . "Lib/libs/weixin/wechat.class.php";
			$client = new WechatClient($weixin_admin['app_id'], $weixin_admin['app_key']);
		}else{
			include_once APP_PATH . "Lib/libs/weixin/wechat.class.php";

			$client = new WechatClient($weixin_a['appid'], $weixin_a['appsecret']);
		}
        return $client;
    }
