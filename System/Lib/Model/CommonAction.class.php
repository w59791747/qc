...
class CommonAction extends Action {
    ...
    protected function _initialize() {
		...
		/*$host_shouquan  = substr($_SERVER['HTTP_HOST'],strpos($_SERVER['HTTP_HOST'],'.')+1);
		$shouquanma = C('shouquan');
		$cache = cache(array('type'=>'File','expire'=> '21600'));
		if(!$shouquan= $cache->get('shouquan')){
			
			$h = base64_encode($host_shouquan);$c = $shouquanma;
			$shouquan = file_get_contents("http://www.izhuchuang.com/index/shouquan/host/".$h."/code/".$c.".html");
			if($shouquan  != 'success'){
				 $this->error('该网站未授权','http://www.izhuchuang.com');
			}
			$cache->set('shouquan',$shouquan);
		}else{
			if($shouquan  != 'success'){
				 $this->error('该网站未授权','http://www.izhuchuang.com');
			}
		...
		if($this->member['city'] == '0' && $this->member['from'] != 'shop'){
			if(strtolower(MODULE_NAME) != 'set' &&  ACTION_NAME != 'index'){
				$this->error('请先去完善资料', U('set/index'));
			}
		}
		D('Order')->check_all('7');
		$this->assign('city',$city);
		$this->assign('citylist',json_encode(D('City')->fetchAll()));
		$this->assign('Province',json_encode(D('Province')->fetchAll()));
		$this->assign('from_list',D('Member')->_from_list());
		$this->assign('shop_list',D('Shop')->fetchAll());
		$this->assign('city_list',D('City')->fetchAll());
		



		$Articlecate = D('Articlecate')->where(array('parent_id'=>3,'hidden'=>0))->limit(3)->select();
