 ...
 */
class  Payment2Action extends CommonAction{
    
    public function index(){
       $Payment2 = D('Payment2');
       import('ORG.Util.Page');// 导入分页类
       $count      = $Payment2->count();// 查询满足要求的总记录数 
       $Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
       $show       = $Page->show();// 分页显示输出
       $list = $Payment2->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function uninstall(){
        ...
        D('Payment2')->save($datas);
        D('Payment2')->cleanCache();
        $this->chuangSuccess('卸载支付方式成功！',U('Payment2/index'));
    }
    ...
    public function install(){
        ...
        if($payments[$payment_id]['code'] == 'money'){
            D('Payment2')->save(array(
                'payment_id'=>$payment_id,
                'is_open' => 1
            ));
            $this->success("安装成功", U('Payment2/index'));die;
        }
        if($this->isPost()){
            ...
            D('Payment2')->save($datas);
            D('Payment2')->cleanCache();
            $this->chuangSuccess('恭喜您安装支付方式成功！',U('Payment2/index'));
        }else{
	...
	public function closemobile($payment_id)
	{
		if(is_numeric($payment_id) && ($payment_id = (int)$payment_id)){
             ...
             $obj->save($data);
             $obj->cleanCache();
             $this->chuangSuccess('操作成功！',U('Payment2/index'));
         }
	}
