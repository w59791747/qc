...
class MenuAction extends CommonAction {
    ...
    public function create($parent_id = 0) {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->chuangSuccess('添加成功', U('menu/index'));
            }
    ...
    public function action($parent_id = 0) {
        ...
        if ($this->isPost()) {
            ...
            $obj->cleanCache();
            $this->chuangSuccess('更新成功', U('menu/index'));
        } else {
            $menu = D('Menu')->fetchAll();
            $this->assign('datas', $menu);
        ...
        if ($menu_id = (int) $menu_id) {
            ...
            foreach ($menu as $val) {
                if ($val['parent_id'] == $menu_id)
                    $this->chuangError('该菜单下还有其他子菜单');
            }
            $obj->delete($menu_id);
            $obj->cleanCache();
            $this->chuangSuccess('删除成功！', U('menu/index'));
        }
