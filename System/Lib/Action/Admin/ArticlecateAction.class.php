...
class ArticlecateAction extends CommonAction {
    ...
    public function index() {
		...
		$m['from'] = 'article';
        $list = $Articlecate->where($m)->order(array('orderby'=>'asc'))->select();
    ...
    public function create($parent_id=0) {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->chuangSuccess('添加成功', U('articlecate/index'));
            }
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->chuangSuccess('操作成功', U('articlecate/index'));
                }
                $this->chuangError('操作失败');
            } else {
    ...
    public function delete($cate_id = 0) {
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->chuangSuccess('删除成功！', U('articlecate/index'));
        } else {
    ...
    public function update() {
        ...
        $obj->cleanCache();
        $this->chuangSuccess('更新成功', U('articlecate/index'));
    }
