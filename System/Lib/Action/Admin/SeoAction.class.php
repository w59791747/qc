...
class SeoAction extends CommonAction {
    ...
    private $create_fields = array('seo_key', 'seo_title', 'seo_keywords', 'seo_desc');
    private $edit_fields = array('seo_key', 'seo_title', 'seo_keywords', 'seo_desc');
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                 $obj->cleanCache();
                $this->chuangSuccess('添加成功', U('seo/index'));
            }
        ...
        if ($seo_id = (int) $seo_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                     $obj->cleanCache();
                    $this->chuangSuccess('编辑成功', U('seo/index'));
                }
                $this->chuangError('操作失败');
            } else {
    ...
    public function delete($seo_id = 0) {
        if (is_numeric($seo_id) && ($seo_id = (int) $seo_id)) {
            ...
            $obj->delete($seo_id);
            $obj->cleanCache();
            $this->chuangSuccess('删除成功！', U('seo/index'));
        } else {
