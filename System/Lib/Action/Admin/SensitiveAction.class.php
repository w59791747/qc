...
class SensitiveAction extends CommonAction{
    ...
    public function create() {
        ...
        if($obj->add($data)){
            $obj->cleanCache();
            $this->chuangSuccess('添加成功',U('sensitive/index'));
        }
        ...
        if($words_id =(int) $words_id){
            ...
            if ($this->isPost()) {
                ...
                if(false!==$obj->save($data)){
                    $obj->cleanCache();
                    $this->chuangSuccess('编辑成功',U('sensitive/index'));
                }
                $this->chuangError('操作失败');
    ...
    public function delete($words_id = 0){
         if(is_numeric($words_id) && ($words_id = (int)$words_id)){
             ...
             $obj->delete($words_id);
             $obj->cleanCache();
             $this->chuangSuccess('删除成功！',U('sensitive/index'));
         }else{
