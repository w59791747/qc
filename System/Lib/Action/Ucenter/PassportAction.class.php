...
class PassportAction extends CommonAction {
    ...
    private $create_fields = array('name', 'passwd','from','fenxiao_id');

    public function reg() {
        if ($this->isPost()) {
            if($this->_CONFIG['power']['reg'] == 'on'){
				$yzm = $this->_post('yzm');
            ...
            if ($password2 !== $data['passwd']) {
                session('verify', null);
                $this->chuangError('两次密码不一致', 3000, true);
            }
            //开始其他的判断了
			if(cookie('fenxiao_id')){
				$data['fenxiao_id'] = cookie('fenxiao_id');
			}
            if (D('Passport')->reg($data)) {
                
                $this->chuangSuccess('恭喜您，注册成功！', U('ucenter/index/index'));
            }
	...
	public function reg2() {
		...
		if ($this->isPost()) {
			...
			$data['name'] = $name = $this->_post('name');
        ...
        if ($this->isPost()) {
            ...
            if (D('Passport')->reg($data)) {
                ...
                $this->chuangSuccess('恭喜您，注册成功！', U('index/index'));
            }
            $this->chuangError(D('Passport')->getError(), 3000, true);
        } else {
			$this->assign('fromlist', D('Member')->_from_list()); 
