        ...
        if(!is_numeric($order_id)){
            ...
            if(!$order = D('Order')->find($order_id)){
				...
				if($status == '100')
				{
					...
					$this->chuangSuccess('支付成功', U('order/lists'));
				}else if($status == '101'){
                ...
                if(strpos($_SERVER["HTTP_USER_AGENT"], 'MicroMessenger') === false && strpos($url,'wxpay')){
					$u = base64_encode($url);
					$qrurl = U('mobile/payment/wxqrcode', array('codeurl'=>$u, 'amount'=>$order['amount'],'order_id'=>$order['order_id']));
					$this->chuangSuccess('立即支付',$qrurl);
                }else if(strpos($_SERVER["HTTP_USER_AGENT"], 'MicroMessenger') && $code == 'wxpay'){
					 $this->assign('jsApiParameters', $url);
					  $this->display();
				}else{
