   ...
   public function news()
   {
	...
	public function deletefollow($follow_id)
	{
		 if($follow_id = (int)$follow_id){
             ...
             $this->chuangSuccess('删除成功！',U('message/follow'));
         }
	...
	public function deletefollow_p($follow_id)
	{
		 if($follow_id = (int)$follow_id){
             $obj =D('Projectfollow');
             $obj->delete($follow_id);
             $this->chuangSuccess('删除成功！',U('message/follow_p'));
         }
         $this->chuangError('删除失败!');
	}

	public function comments($from='1')
	{
		...
		if($from == '1'){
			...
			$map['comment_uid'] = $this->uid;
			$map['audit'] = '1';
		}	
		$count      = $Membercomment->where($map)->count();// 查询满足要求的总记录数 
		$Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
		$show       = $Page->show();// 分页显示输出
		$list = $Membercomment->where($map)->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->assign('from',$from);
		
		if($from == '1'){
			$this->itemsby($list,'comment_uid','Member','memberlist');
		}else{
			$this->itemsby($list,'uid','Member','memberlist');
	...
	public function comments_article()
	{
		...
		$Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
		$show       = $Page->show();// 分页显示输出
		$list = $Articlecomment->where($map)->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->itemsby($list,'article_id','Article','articlelist');
		$this->assign('list',$list);// 赋值数据集
		$this->assign('page',$show);// 赋值分页输出
		$this->display(); // 输出模板
	}

	public function comments_project($from='1')
	{
		...
		if($from == '1'){
			...
			$map['project_id'] = array('IN',substr($ids,'0','-1'));
			$map['audit'] = '1';
		}	
	...
	public function commentsdelete($comment_id = 0)
	{
		 if (is_numeric($comment_id) && ($comment_id = (int) $comment_id)) {
            $obj = D('Membercomment');
			if ($this->audit_delete($comment_id,'Member','Membercomment','delete')) {
				$this->chuangSuccess('删除成功！', U('message/comments'));
			}
        }
	}
	...
	public function commentsarticledelete($comment_id = 0)
	{
		 if (is_numeric($comment_id) && ($comment_id = (int) $comment_id)) {
			$obj = D('Articlecomment');
			if ($this->audit_delete($comment_id,'Article','Articlecomment','delete')) {
				$this->chuangSuccess('删除成功！', U('message/comments_article'));
			}
        }
	}
