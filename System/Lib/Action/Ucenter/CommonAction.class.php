...
class CommonAction extends Action {
    ...
    protected function _initialize() {
        ...
        $this->uid =  getUid();
		if($_POST && empty($this->uid) && ACTION_NAME != 'login' && ACTION_NAME != 'reg'){
			$this->chuangError('请先登录');
		}
        if (empty($this->uid) && strtolower(MODULE_NAME) != 'passport') {
        ...
        if(empty($this->member) && strtolower(MODULE_NAME) != 'passport'){
            header("Location: " . U('mobile/passport/login'));
            die;
        }

		$Articlecate = D('Articlecate')->where(array('parent_id'=>111,'hidden'=>0))->order(array('orderby'=>'asc'))->limit(6)->select();
		...
		if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')!==false && $this->uid){
...
public function audit_comment($comment_id,$table1,$table2,$role,$fenxiao,$fenxiao_role) 
    ...
    private function parseTemplate($template = '') {
        ...
        define('NOW_PATH',BASE_PATH.'/themes/'.$theme.'Ucenter/');
        // 获取当前主题的模版路径
        define('THEME_PATH', BASE_PATH . '/themes/default/Ucenter/');
        define('APP_TMPL_PATH', __ROOT__ . '/themes/'.$theme.'Ucenter/');
