...
class PacketAction extends CommonAction {
	...
	public function my_packet($id,$page = 1){
		...
		if ($items) {
			...
			foreach($items as $k => $v){
				if($opens[$v['open_id']]){
					...
					$user = D('Member_weixin')->where(array('open_id'=>$v['open_id']))->find();
	...
	public function is_packet($id,$page = 1){
	...
	public function reward_sub($sn_id,$packet_id,$pwd,$mobile){
		...
		$filter = array('open_id'=>$openid,'packet_id'=>$packet_id);	
		$filter['id']	= $sn_id;
