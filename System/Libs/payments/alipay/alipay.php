...
class Payment_Alipay 
{
    ...
    public function return_verify()
    {
		...
		if(defined('IN_MOBILE')){
			if(empty($_GET)) {//判断GET来的数组是否为空
				...
				$this->_logs($log);
				if ($mysign == $_GET["sign"]){
					return array('trade_no'=>$notify['out_trade_no'], 'pay_trade_no'=>$notify['order_no'], 'trade_status'=>$_POST['trade_status'], 'amount'=>$notify['total_fee'], 'extra_param'=>$this->_decode_params($notify['extra_common_param']));
				}
				return false;
			}
    ...
    public function build_parameter($params)
    {
        ...
        if ($this->payment['payment_config']['alipay_service'] == 'create_direct_pay_by_user'){
            ...
            $parameter['price'] = sprintf("%01.2f", $params['amount']/100);
    ...
    protected function _build_query($params, $urlencode=true)
    {
		...
		//if(strpos($query_string,'buyer_id') !== false){
    ...
    private function _filter_params($params)
    {
        $para = array();
        while (list ($key, $val) = each ($params)) {
            if($key == "sign" || $key == "sign_type" || $val == "" || $key == "_URL_"|| $key == "from"|| $key == "code") continue;
