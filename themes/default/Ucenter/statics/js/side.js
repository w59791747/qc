...
documentReady(function() {
	...
	{	
			...
			$('#menuIco').bind('touchstart touchon', function(event){
			...
			$('.a_submenu').bind('touchstart touchon', function(event){
				$(this).find('.submenu').toggle(250);
			});
		}
		else
		{
			
			$('#menuIco').bind('click', function(event){
					if(aMenuClicked)
					{
						$('.menu').removeClass('activeState');
						$('.contentWrapper').removeClass('moved');
					...
					{
						$('.menu').addClass('activeState');
						$('.contentWrapper').addClass('moved');
						$('.menuWrapper').addClass('show');
						aMenuClicked = true;
					}
			});
			
			$('.a_submenu').bind('click', function(event){
				$(this).find('.submenu').toggle(250);
			});
